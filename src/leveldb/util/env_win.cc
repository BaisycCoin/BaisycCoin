...
std::wstring GetLastErrSzW()
{
    ...
    std::wstring Err = (BSYCWSTR)lpMsgBuf;
    LocalFree(lpMsgBuf);
    return Err;
}
...
BOOL Win32RandomAccessFile::_Init( BSYCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
...
BOOL Win32MapFile::_Init( BSYCWSTR Path )
{
...
BOOL Win32FileLock::_Init( BSYCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
